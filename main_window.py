                ...
                try:
                    if self.tracker:
                        ...
                        if landmarks and len(landmarks) >= 21:
                            ...
                            if gesture != "no_hand" and confidence > 0.6:
                                # Draw gesture name
                                cv2.putText(frame, f"{gesture}", (20, 40), 
                                          cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                                # Draw confidence bar
                                bar_width = int(confidence * 100)
                                cv2.rectangle(frame, (20, 50), (20 + bar_width, 60), 
                                            (0, 255, 0), -1)
                                cv2.rectangle(frame, (20, 50), (120, 60), 
                                            (255, 255, 255), 1)
...
class CanvasWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.grid_enabled:
            ...
            painter.fillRect(visible_rect, Qt.white)
    ...
    def mousePressEvent(self, event):
        try:
            ...
            if event.button() == Qt.LeftButton:
                ...
                if self.engine.mode in ['shape', 'select']:
                    self.engine.start_stroke(canvas_pos.x(), canvas_pos.y())
                else:
                    self.engine.start_stroke(canvas_pos.x(), canvas_pos.y())
                
                self.canvasClicked.emit(canvas_pos, event.button())
    ...
    def mouseMoveEvent(self, event):
        try:
            ...
            if self.drawing and self.last_point:
                if self.engine.mode in ['shape', 'select']:
                    # For shapes and selection, we need to update the preview
                    self.engine.continue_stroke(canvas_pos.x(), canvas_pos.y())
                else:
                    self.engine.continue_stroke(canvas_pos.x(), canvas_pos.y())
                    self.last_point = canvas_pos
                self.update_canvas()
    ...
    def mouseReleaseEvent(self, event):
        try:
            ...
            self.canvasReleased.emit(canvas_pos, event.button())
...
class MainWindow(QMainWindow):
    ...
    def init_components(self):
        ...
        try:
            ...
            self.gesture_controller = GestureController(self.config, self.calibrator)
    ...
    def create_toolbar(self):
        ...
        # Increased webcam display size
        self.webcam_label = QLabel()
        self.webcam_label.setFixedSize(320, 240)  # Increased from 200x150
        ...
        """)
        self.webcam_fps_label.setAlignment(Qt.AlignLeft | Qt.AlignTop)
        
        # Create overlay layout
        webcam_overlay = QWidget(self.webcam_label)
        overlay_layout = QVBoxLayout(webcam_overlay)
        overlay_layout.setContentsMargins(5, 5, 5, 5)
        overlay_layout.addWidget(self.webcam_fps_label)
        ...
        webcam_layout.addWidget(self.gesture_checkbox)
    ...
    def create_sidebar(self):
        ...
        calibrate_btn = QPushButton("🎯 Calibrate Gestures")
        calibrate_btn.clicked.connect(self.calibrate_gestures)
    ...
    def update_webcam_display(self, frame):
        ...
        try:
            # Resize for display (match the label size)
            target_width = self.webcam_label.width()
            target_height = self.webcam_label.height()
    ...
    def process_gesture_commands(self):
        ...
        try:
            ...
            if hasattr(self.camera_worker, 'landmarks') and self.camera_worker.landmarks:
                ...
                # Get index finger tip for precise drawing
                index_tip = self.gesture_controller.get_index_tip_position(landmarks)
                if not index_tip:
                    index_tip = hand_position
                
                # Map webcam coordinates (640x480) to canvas coordinates (1280x720)
                webcam_to_canvas_x = self.engine.w / 640.0
                webcam_to_canvas_y = self.engine.h / 480.0
    ...
    def update_canvas_display(self):
        """Update canvas display periodically"""
        if self.canvas_widget.needs_redraw:
    ...
    def toggle_shape_fill(self, state):
        """Toggle shape fill"""
        self.fill_shape = (state == Qt.Checked)
        self.engine.fill_shape = self.fill_shape
    ...
    def toggle_webcam(self, state):
        ...
        self.webcam_visible = (state == Qt.Checked)
        if not self.webcam_visible:
            self.webcam_label.setText("Webcam\nDisabled")
    ...
    def toggle_gesture_mode(self, state):
        """Toggle gesture mode"""
        self.gesture_mode = (state == Qt.Checked)
        ...
        if self.gesture_mode:
            ...
            self.mode_label.setText("Mode: Mouse")
            self.status_bar.showMessage("Mouse mode enabled", 2000)
    ...
    def test_gesture_drawing(self):
        ...
        QMessageBox.information(self, "Gesture Test",
                              ...
                              "1. Show OPEN HAND to the camera for calibration\n"
    ...
    def calibrate_gestures(self):
        """Calibrate gestures"""
        QMessageBox.information(self, "Gesture Calibration",
                              "Gesture calibration will be implemented soon.\n\n"
                              "For now, the system learns your gestures automatically.")
