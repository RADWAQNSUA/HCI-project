...
class GestureController:
    ...
    def update_hand_normalization(self, landmarks):
        ...
        if landmarks and len(landmarks) >= 10:
            ...
            if len(self.hand_size_history) >= 3:
                ...
                self.normalized_hand_size = weighted_size
            else:
                self.normalized_hand_size = current_size
    ...
    def calculate_hand_size(self, landmarks):
        ...
        # If calibrator exists, use it for normalization
        if self.calibrator and hasattr(self.calibrator, 'get_base_hand_size'):
            base_size = self.calibrator.get_base_hand_size()
            if base_size > 0:
                return hand_size / base_size * 100
    ...
    def get_smooth_position(self, landmarks):
        ...
        self.kalman_x.measurement_variance = max(0.01, min(0.2, 1 / (len(self.position_history) + 1)))
        self.kalman_y.measurement_variance = max(0.01, min(0.2, 1 / (len(self.position_history) + 1)))
